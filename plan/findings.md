...
## Phase 3 trigger — OmniAgents `function_call without reasoning` blocks list→promote on gpt-5.2

- **Symptom (2026-05-25 22:09 CDT, worktree 9a0b0):** With a freshly-wiped `sessions.db`, the user tried "List 5 lead candidates with no filters, then promote the first one." Turn 1 (`list_lead_candidates`, auto-approved, single tool call) completed and returned 5 McAllen candidates. Turn 2 ("promote #1") went silent. Server log (`/tmp/agent-server.log` at 16:30:45) shows:
  ```
  openai.BadRequestError: 400 — Item 'fc_0e21ed20…' of type 'function_call' was
  provided without its required 'reasoning' item: 'rs_0e21ed20…'.
  Error context — Session c3fbd1e9…, Turn: 0, Total items sent: 1,
  From DB: 6, Added this turn: -5
  ```
- **Root cause:** OmniAgents persists turn-1 history to SQLite (`~/.omniagents/sessions/default/rgv_lead_scraper/sessions.db`). On turn 2, its compaction pipeline reads the 6 items but strips 5 of them — including `reasoning` items paired with `function_call` items. Azure gpt-5.2 strict-mode validates that every `function_call` is preceded by its `reasoning` sibling and rejects the malformed input. Reasoning-model artifact: doesn't affect non-reasoning models (e.g. gpt-4.1).
- **Workaround tried (insufficient):** wipe `sessions.db` between turns. Works for exactly one turn before reaccumulating. Not viable as ongoing UX.
- **Architectural escape:** collapse the agent's mutating surface to a **single** tool. With only `request_lead_generation` as the mutator (gated, single-turn), no cross-turn `function_call`/`reasoning` reload happens for any mutation path. `list_lead_candidates` (read-only, single tool call per turn) does not trigger the bug because the next turn doesn't need to reference its prior function_call.
